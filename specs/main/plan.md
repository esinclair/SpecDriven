...
✅ **Performance budget**: VERIFIED
   - All synchronous operations designed for ≤1s response time
...
**Constitution Compliance**: ✅ 100% PASS - All constitutional principles satisfied
...
└── checklists/          # Quality gates and requirements tracking
    └── requirements.md
```
...
```text
SpecDriven/
├── build.gradle                    # Gradle build with OpenAPI generation
├── settings.gradle
...
│   │   │       └── ErrorResponseFactory.java       # Stable error code generation
│   │   │
│   │   └── resources/
│   │       ├── application.yml                      # Spring Boot configuration
│   │       ├── openapi.yaml                         # OpenAPI contract (source of truth)
│   │       └── db/migration/                        # Flyway SQL migrations
│   │           ├── V001__init_schema.sql           # Users table
│   │           ├── V002__roles_permissions.sql     # Roles and permissions
│   │           └── V003__users_api.sql             # User-role mappings
│   │
│   ├── main-gen/                                    # Generated code (do not edit manually)
│   │   └── java/com/example/specdriven/
│   │       └── api/
│   │           ├── LoginApi.java                    # Generated login API interface
│   │           ├── PingApi.java                     # Generated health check API
...
│   │           │   ├── Permission.java
│   │           │   └── RoleName.java
│   │           └── invoker/                         # Generated invoker (minimal)
│   │
│   └── test/
│       ├── java/com/example/specdriven/
...
│       │   │   ├── RoleManagementIntegrationTest.java
│       │   │   └── FeatureFlagIntegrationTest.java
...
│       │   │   └── FeatureFlagSecurityFilterTest.java
...
- **Implementation**:
  - `UserMapper.toEntity(CreateUserRequest)` → `UserEntity`
  - `UserMapper.toDto(UserEntity)` → `User`
...
- ✅ Get user by ID returns 200 with user data
- ✅ Update user with valid data returns 200
- ✅ Delete user returns 204
- ✅ List users with pagination returns 200 with page
- ✅ Assign role returns 204
- ✅ Remove role returns 204

#### Negative Tests (Every Endpoint)

- ✅ Validation failures return 400 with VALIDATION_FAILED
- ✅ Not found returns 404 with RESOURCE_NOT_FOUND
- ✅ Authentication failures return 401 with AUTHENTICATION_FAILED
- ✅ Conflicts (duplicate email) return 409 with CONFLICT
- ✅ Feature flag disabled returns 404 (no disclosure)
...
- ✅ Role filtering in list users
- ✅ Concurrent updates to same user

#### Performance Tests

- ✅ Health check < 1s for 95% of requests
- ✅ Login < 1s for 95% of requests
- ✅ User CRUD operations < 1s for 95% of requests
- ✅ Paginated list < 1s for 95% of requests
...
2. User mapper (API DTOs ↔ domain entities)
...
- [ ] Tests requiring authentication follow proper pattern (create user if needed → login → get token → use token in request)
...
2. **Testing**
   ...
   - Performance: ≤1s budget validated
...
- Performance tests validate ≤1s budget with representative data sizes

## Next Steps

1. ✅ **Phase 0 Complete**: Research decisions documented in research.md
2. ✅ **Phase 1 Complete**: Data model and OpenAPI contract defined
   - ✅ OpenAPI contract created: `specs/main/contracts/openapi.contract.yaml`
...
**Current State**:
- ✅ Planning complete with architecture, design patterns, and implementation approach
- ✅ OpenAPI contract validated and generating Java interfaces/models
- ✅ Configuration files in place (application.yml, openapi.yaml)
- ⏳ Application implementation pending (controllers, services, repositories, security)
